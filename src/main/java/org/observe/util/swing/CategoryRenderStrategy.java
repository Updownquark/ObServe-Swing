...
public class CategoryRenderStrategy<R, C> implements ValueRenderer<R> {
	public class CategoryMutationStrategy {
		...
		public CategoryMutationStrategy editableIf(BiPredicate<? super R, ? super C> editable) {
			return editableIf(
				FunctionUtils.printablePred(cell -> editable.test(cell.getModelValue(), cell.getCellValue()), editable::toString, editable));
		}
		...
		public CategoryMutationStrategy withRowValueSwitch(BiFunction<? super R, ? super C, ? extends R> rowMutator) {
			theRowMutator = FunctionUtils.printableBiFn((el, cv) -> rowMutator.apply(el.get(), cv), rowMutator::toString, rowMutator);
			return this;
		}
		...
		public CategoryMutationStrategy asText(Format<C> format) {
			...
			if (isRenderDefault)
				formatText(FunctionUtils.printableFn(format::format, format::toString, format));
			return this;
		}
		...
		public CategoryMutationStrategy asText(Function<? super R, Format<C>> format) {
			...
			if (isRenderDefault)
				formatText(FunctionUtils.printableBiFn((r, c) -> format.apply(r).format(c), format::toString, format));
			return this;
		}
